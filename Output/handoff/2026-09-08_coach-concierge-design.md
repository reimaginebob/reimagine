...
**Correction, 2026-09-09 (`Output/handoff/2026-09-09_coach-engine-guardrails-brief.md`).** "I'm good for now" (session-level quiet) does not currently carve out significant moments the way this section and 3a describe — the shipped evaluator's quiet check (`if(quietUntilReload||quietScreens[step])return`) blocks every family uniformly, and no entry in the catalog carries anything but a single, always-'open' `significance` value; the ordinary/significant column described above was never actually coded. Discovered during that brief's premise-verification, which needed the distinction for a mood-hold rule (below) and found it wasn't there. Building the real significance column so "significant moments still open" becomes literally true is deferred to its own future brief (Bob's call, 2026-09-09: the current one-tap-quiets-everything behavior is already the quietest state the product can be in, so there is no urgency pulling this forward, and building it changes real behavior — significant moments would start reopening the panel again after one tap — which deserves its own decision rather than riding in on a guardrails PR).

**Engine guardrails, 2026-09-09 (`Output/handoff/2026-09-09_coach-engine-guardrails-brief.md`).** Two session-scoped holds sit in front of the evaluator, neither expressed as a catalog column:

- **Distress hold (hard).** `ensureDistressSupport` (`src/text-strippers.js`) already appends a human-pointer sentence to a typed reply when the person's own message matches an explicit self-harm/not-wanting-to-be-alive pattern — but it only ever sees a real typed message, so a silent moment turn (which carries no user text) could still fire proactively right on top of that reply. The server now also emits a plain `X-Coach-Distress: 1` header whenever the trigger matched (independent of whether the pointer sentence had to be appended), and the client sets a `coachDistressHold` session flag from it. The evaluator checks this flag first, before even the quiet-state check — no moment fires, in any family, while it is set. Clears at the next session open.
- **Discouragement hold (soft).** The DISCOURAGEMENT response (`SYSTEM_PROMPT_HEAD`) now ends with a silent `MOOD: low` trailer, stripped the same way `SELFCHECK:` is (`parseMood`, `src/coach-routing.js`), whenever the model actually used that response. The server emits `X-Coach-Mood: low`; the client sets `coachMoodHold`. While set, the evaluator holds every family except `delivery` and `choice` — a literal enumeration (per the correction above, there was no significant/ordinary column to key this off of instead) matching "the moments that respond to something the person just did keep firing; everything that would interrupt on its own initiative holds." Clears at the next session open.

Neither hold is announced to the person; both are pure engine-side gating.

**Situation cost, sampled.** `buildCoachRequest` (`api/coach.js`) now logs the Situation-derived text's size (the in-focus expansion plus the "on step X" context note — the two pieces of `situation` this function actually renders) on roughly 1 in 20 turns, `Math.ceil(chars/4)` as an estimated-token figure. Confirmed the block's placement is already correct relative to the #807 cache reorder: `contextNote` rides on the always-uncached final user turn, and `expansion` folds into `profileBlock`, which was already last in the `system` array's cache_control chain — nothing to fix there, only the missing cost visibility.
...
**Phase 2: Moments engine with Arrival, Choice, Delivery on Career Paths and Put It to Work.** One evaluator, one dedupe store, the significance column, three families (the dismissal affordances arrive with the presence model in Phase 1). Delivery reuses the Brand richness prompt shape. The Put It to Work routing question ships here as the Arrival moment (its exact copy, and every other Phase 2 moment template's, was rewritten 2026-09-09 per the voice review — see Section 12 decision 4 and `Output/handoff/2026-09-09_coach-voice-review.md`). Existing onboarding triggers untouched.
...
4. **Put It to Work routing copy.** Confirmed 2026-09-08, revised 2026-09-09 per the voice review (`Output/handoff/2026-09-09_coach-voice-review.md`, Section 1) — the original wording read as AI-speak ("moving," "direction," "build from your brand" were product shorthand, not the user's own words). Current approved wording: "Are you working on any job opportunities right now, like an application you've sent, someone who offered to refer you, or an interview coming up? If so, let's start with that one. If not, we'll look at the kinds of roles that fit you and build from there." Taps: "Yes, I have one" / "Not yet." Replaced the existing `orientation-route` line at `src/App.jsx:9085`; the two-door screen's own callout was retired in the same PR (Phase 2a) so the two pitches never coexist.
