...
Two things to know about it. A `deployment_status` workflow always runs the version on the **default branch**, so a change to this workflow does not take effect until it is merged — including the PR that first adds it. And the manual path below still works and is still the right tool when CI is unavailable or you want to smoke something by hand.
