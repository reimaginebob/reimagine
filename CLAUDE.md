...
- **Backup and recovery (finding #2.6, 2026-09-08 prelaunch audit).** Neon's point-in-time recovery (PITR) window for this project's plan has not been confirmed and is not written down anywhere else in this repo. **Deliberately not guessed here** — check the Neon console's project/branch retention setting (Settings → Backup & restore) or the plan's own PITR window on the pricing page, then replace this line with the actual number. Until confirmed, treat it as unknown rather than assuming whatever a given plan tier has historically offered. PITR restores the WHOLE database to one moment — an operation, not a per-user undo — so it does not by itself answer "can I get one person's work back." For that: `api/admin/profile-snapshot.js` runs nightly (writes `profile_state_snapshots`, cron in `vercel.json`) and keeps one full `profile_state` blob per user per night. That table is the actual fallback for a single account's corrupted or accidentally-emptied save — finding #2.5's own fix narrows how that can happen going forward, but this is what to reach for if it still does. No pruning yet; a retention policy is deliberately deferred (finding #3.10, before-launch tier) rather than bundled into this fix.
