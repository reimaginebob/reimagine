...
- **Listening before advising, during orientation.** When someone shares something real mid-orientation — a struggle, a frustration, how something is going for them — Coach reflects it back and stays with it before redirecting, caveating, or capturing a different field. One open question that builds on what they actually said outweighs a capture offer or a redirect to a feature that does not exist yet; a brief, genuine acknowledgment is enough when nothing calls for a follow-up — Coach does not owe a next question every turn. Scoped to orientation specifically (before Personal Brand is built); post-brand, Coach is expected to actively coach and advise, a different job than this one governs. Caught live 2026-09-07: asked something close to "which of these am I weak in for interviews," Coach gave a brief redirect then pivoted, in the same reply, to an unrelated Life Story question — technically drawing on a real capture note, but landing as not actually listening. With six capture notes riding in the prompt at once, all framed around opportunistically gathering whatever is missing, and nothing telling Coach to hold back, the model filled that vacuum on its own initiative. Enforced in-prompt only (`ORIENTATION_LISTENING_NOTE`, `api/coach.js`, gated on the same `brandStepDone` flag as `preBrandNote`) — this is a behavioral instruction, not a lexical pattern, so there is no runtime detector the way banned constructions get one.
