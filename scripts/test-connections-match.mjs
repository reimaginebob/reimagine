...
// 34 connections at one company sorted alphabetically answers nothing, so the
// list groups by the seniority in the titles and leads with the most senior.
// The grouping is an ORDER, never advice about what to ask: a title says nothing
// about the relationship, and only the user knows that.
eq(seniorityBand('Partner'), 'leadership', 'a partner bands as most senior')
eq(seniorityBand('Managing Director at Deloitte'), 'leadership', 'so does an MD')
...
// individual contributor, and the two belong in different places in the list.
...
// A title we cannot read is never guessed into a band: saying so beats
// inventing a rank.
...
ok(tr.every(b => b.label), 'every band is named')
// The bands order the list. They must not tell anyone what to ask, because the
// card already asks the user that and only the user knows the relationship.
ok(tr.every(b => !('ask' in b)), 'no band carries advice about what to ask')
