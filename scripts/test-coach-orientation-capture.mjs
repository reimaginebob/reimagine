...
check(coach.includes('${VALUES_CAPTURE_NOTE}${ASSESSMENT_CAPTURE_NOTE}${reputationCaptureNote}${skillsCaptureNote}`'),
  `${COACH}: reputationCaptureNote/skillsCaptureNote are not appended in the empty-profile template`)
check(/\$\{VALUES_CAPTURE_NOTE\}\$\{ASSESSMENT_CAPTURE_NOTE\}\$\{reputationCaptureNote\}\$\{skillsCaptureNote\}\$\{searchIntakeNoteThisTurn\}/.test(coach),
