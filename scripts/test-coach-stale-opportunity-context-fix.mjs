...
check((app.match(/pursuitOfferMessage\(coachSaveTarget\(\)\.title,coachSaveTarget\(\)\.id\)/g) || []).length === 2,
  `${APP}: expected both Chat mounts to pass coachSaveTarget().id into pursuitOfferMessage -- the offer must be built with the target that was actually in focus when it was shown`)
