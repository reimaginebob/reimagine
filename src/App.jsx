...
import { stripCoachSpeak, applyContaminationPlaceholders, stripLogicFlipCadence, stripSincerityQualifiers } from "./text-strippers.mjs"
...
// render). Belt-and-suspenders for the audience-placeholder "rooms"
// construction, whose prompt-source enforcement the model still sometimes
// overrides under generation pressure. Telemetry via console.warn fires
// when the filter strips at least one occurrence, so post-ship monitoring
// shows how often the model is slipping past the prompt rule. Pure-string,
// idempotent, safe on JSON payloads. The function body intentionally
// contains the banned construction as its own match target, so the
// surrounding voice-allow markers exempt this region from the build-time
// scan. (Em-dash stripping was removed 2026-05-27 when Bob dropped the
// em-dash ban for Reimagine outputs; em dashes are now normal punctuation
// the model can use freely.)
// voice-allow
function stripRoomsPlaceholder(text) {
...
// Post-processing for AI-meta-narration. Same shape as stripRoomsPlaceholder:
