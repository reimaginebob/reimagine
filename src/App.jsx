...
<div class="footer"><span>Reimagine by Career Club · career.club</span><span>${esc(date)}</span></div>
  ...
  const buildDoor2Record=(id,title,jdText)=>{
    const ts=new Date().toISOString()
    return{id,title,lane:'specific',source:'door2',createdAt:ts,updatedAt:ts,outputs:{op:outputs.op||''},done:done.includes('op')?['op']:[],feedback:{op:feedback.op||''},upstream:buildUpstreamSnapshot(),jd:jdText||'',schemaVersion:2,sections:{p5:{content:'',builtAt:null},p6:{bridge_story:null,user_picks:null,user_freeform:'',builtAt:null},p_res:{content:'',builtAt:null},p11:{content:'',builtAt:null}}}
  }
  ...
  const generateOpSection=async(key)=>{
    ...
    try{
      const fn=()=>correctionsBlock(profile.corrections)+(key==='p5'?P.p5(pc,outputs,chosen,'',jd):key==='p_res'?P.p_res(pc,outputs,chosen,jd):P.p11(pc,outputs,chosen,jd))
      ...
      setSavedPlaybooks(prev=>prev.map(rec=>{
        ...
        sections[key]={...(sections[key]||{}),content:r,builtAt:new Date().toISOString()}
