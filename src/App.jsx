...
At major judgment-call moments where you are interpreting fit, alignment, or value, invite the user to react. Roughly 1-2 invitations per major output section, varied phrasing, placed at the end of an interpretive claim after the user has absorbed the read. Place one invitation at the end of each STAR story's Strengthen section, or one consolidated invitation at the end of The Remix section where the user is reacting to multiple stories at once.
...
EVIDENCE-BASED CONFIDENCE, NOT COMPARISON: Confidence in this output is grounded in the specific evidence the user has surfaced about themselves: accomplishments with numbers, named relationships, specific decisions, the user's own words from orientation. Never elevate the user by contrast to unnamed others. Banned rhetorical moves include any sentence that claims the user is better than "most people," "most others," "the typical X," "the average X," or any unnamed group. Banned regardless of position in the sentence (start, middle, after "when," after "than," after "unlike"). State the evidence in specific detail and let the listener draw the conclusion. The source of any banned framing is not relevant; even if the user's assessment or reputation input contains comparison-based phrasing, paraphrase to the underlying evidence without the comparison. The goal is winsome and likeable, not arrogant and cocky. The user's career deserves confidence; their confidence comes from what they have done, not from claims about how they stack up against others.`,
  op:(pr,outs,sel,jd)=>`Build a playbook for this specific opportunity, weaving in the user's foundation work. The user has completed their full Reimagine journey and chose this path: **${sel||'their target direction'}**.\n\nJOB DESCRIPTION (uploaded or pasted by the user):\n${jd}\n\nUSER'S FOUNDATION WORK:\nRESUME ANALYSIS: ${outs.p1||''}\nWIRING & COMPASS: ${outs.p2||''}\nBRAND SYNTHESIS: ${outs.p3||''}\nBRIDGE STORY: ${outs.p6?outs.p6.substring(0,2000):''}\nGO-TO-MARKET (excerpt): ${outs.p7?outs.p7.substring(0,1500):''}\n\nThe three Wide View lanes are listed below. Your first job is to determine which lane this JD fits, using the four-step method that follows.\n\nLANE-FIT DETERMINATION (do this internally before producing output; do not include the four-step analysis in user-facing content):\n\nStep 1. Read the JD carefully. Extract:\n- Function (what kind of work)\n- Industry (what sector)\n- Position in value chain (operator inside the institution, vendor or agency to the institution, advisor, product builder, investor, etc.)\n- Scale (company size, role scope)\n- Pace (steady institution, growth-stage, venture pace)\n- Mission orientation (mission-rich, mission-light, mission-neutral)\n\nStep 2. Read the three lanes from the WIDE VIEW LANES content listed below. Each lane has a rationale for what kind of work fits there. Map the JD's six dimensions against each lane's frame.\n\nStep 3. Determine which of the three lanes the JD most naturally belongs to. The lane with the strongest dimensional overlap is the lane the role fits. If two lanes are close, pick the lane where the role's PRIMARY reason to consider it is most resonant (Familiar Ground for direct track-record fit, Industry Insider for ecosystem-knowledge fit, Work That Matters for mission and values fit).\n\nStep 4. Compare the determined lane to the user's chosen direction (provided as ${sel||'their target direction'}). Pick one of the three output branches below based on the comparison.\n\nNever show this four-step analysis in user-facing content. The output contains only the analysis of the role, framed by the lane that fits.\n\nWIDE VIEW LANES (the three career-direction frames generated for this user, with their rationale):\n${outs.p4||''}\n\nOUTPUT BRANCHES (select exactly one based on the lane-fit determination above):\n\nBRANCH A: LANE MATCH ALIGNS WITH CHOSEN DIRECTION. The determined lane matches the user's chosen direction. The user is looking at a role that fits the path they chose. Lead the Quick Takeaway with: this role aligns with [user's chosen direction] and fits the [lane name] lane from your Wide View.\n\nBRANCH B: LANE MATCH DIVERGES FROM CHOSEN DIRECTION. The determined lane is one of the user's other two lanes, not their chosen direction. Do not label the role with single-frame verdicts. Surface the divergence as information the user can act on, and make the case for the role on the determined lane's terms. Lead the Quick Takeaway with: this role lives on the [determined lane] lane from your Wide View, not your chosen direction of [chosen].\n\nBRANCH C: JD DOES NOT FIT ANY LANE CLEANLY. The JD has weak dimensional overlap with all three lanes. Do not force-fit. Acknowledge the off-frame nature, name the role's distinctive dimensions, and invite the user to add context. Lead the Quick Takeaway with: this role sits outside the three Wide View lanes generated for your profile.\n\nSTART your response with:\n## QUICK TAKEAWAY\nWrite the Quick Takeaway for the branch you selected. Plain language, no headers inside this section. The first sentence is a literal fill-in template that you reproduce verbatim with the bracketed variables filled in; the rest of the Quick Takeaway elaborates from there.\n\n- BRANCH A: 4 to 5 sentences. The first sentence of the Quick Takeaway must be exactly: "This role aligns with your chosen direction and lives on the [determined lane name] lane from your Wide View." Fill in the bracketed variable with the lane name you determined. No paraphrase. After the lead sentence: state why the role is a strong fit (1 to 2 sentences). Name the most important watch-out or trade-off (1 sentence). End with the action this week (1 sentence).\n- BRANCH B: 5 to 6 sentences. The first sentence of the Quick Takeaway must be exactly: "This role lives on the [determined lane name] lane from your Wide View, not your chosen direction of [user's chosen direction]." Fill in the bracketed variables. No paraphrase. After the lead sentence: make the case for the role on the determined lane's terms (2 to 3 sentences naming the specific reasons it fits that lane). Name the trade the user would be making by pursuing this (1 to 2 sentences: what you would be expanding into, what you would be moving away from). End with a question for the user about whether the broaden is worth it, or an action this week if the user has signaled openness to other lanes (1 sentence).\n- BRANCH C: 4 to 5 sentences. The first sentence of the Quick Takeaway must be exactly: "This role sits outside the three Wide View lanes generated for your profile." No paraphrase. After the lead sentence: name what makes it distinctive (1 to 2 sentences: which dimensions it stretches on, which lane it is closest to without quite fitting). Name the question: is this a role worth exploring as a lateral or surprising direction, or one to pass on? End with an invitation to add context if the user wants the system to reconsider (1 sentence).\n\nThen produce the full playbook in this exact order, with the exact section headers shown. Adapt the analysis (especially WHY THIS COULD BE A FIT and WHAT TO WATCH OUT FOR) to the determined lane per the PER-LANE ANALYSIS TEMPLATES below:\n\n## 1. ALIGNMENT\nName the lane the role fits and the user's chosen direction. If the lanes match (Branch A), state that the role aligns with the user's path on that lane. If the lanes differ (Branch B), name the divergence in lane terms (your chosen direction is [chosen] on the [chosen lane]; this role lives on the [determined lane] lane) and make the case for the role on its own lane's terms. If the JD does not fit any lane cleanly (Branch C), say so and invite the user to add context. Do not block, pause, or warn. Coach, do not gatekeep. The user decides whether to pursue.\n\n## 2. WHY THIS COULD BE A FIT\n2 to 3 paragraphs grounded in specific evidence from their Wiring, Brand Synthesis, and prior wins. Adapt the anchoring to the determined lane (see PER-LANE ANALYSIS TEMPLATES below): Familiar Ground anchors in direct track-record evidence, Industry Insider anchors in ecosystem fluency and insider knowledge, Work That Matters anchors in mission alignment and values resonance. Name the capabilities and the proof points. Concrete, not abstract.\n\n## 3. WHAT TO WATCH OUT FOR\nWhere the role stretches the user past their proven track record. Where the JD might be overselling. Be direct. The user is going to read this and use it to prepare; they need the watch-outs more than the cheerleading.\n\n## 4. THE MOST IMPORTANT SIGNALS IN THIS JD\nIdentify 4 to 6 things in the posting that carry real weight. Everything else is secondary. State each in one sentence and explain why it matters.\n\n## 5. WHAT THE HIRING MANAGER IS SOLVING FOR\nRead the JD as evidence of an underlying problem. Infer from industry, company size, title altitude, and what the posting emphasizes and omits. The user reading this should know what conversation the hiring manager actually wants to have.\n\n## 6. TAILORED FRAMING OF YOUR BRAND SYNTHESIS\nForeground the proof points and angles from the user's Brand Synthesis that carry the most weight for THIS role. Do not rewrite the synthesis. Pick what to emphasize. 3 to 5 specific framing moves with one-line rationale each.\n\n## 7. REMIXED STAR STORIES\nBuild exactly 3 STAR stories remixed for this specific opportunity. T stands for Thinking, not Tasks. The remixing concept: the user already has core stories from their experience; this section selects three and re-emphasizes them for the questions this role's interview cycle is most likely to ask. Use this exact structure for each:\n\n### STORY [NUMBER]: [Short descriptive title]\n**Best for answering:** [2 to 3 specific interview questions this story handles well]\n\n**Situation:** 2 to 3 sentences setting the scene.\n\n**Thinking:** 3 to 4 sentences on how the user diagnosed the situation, what options they weighed, and why they chose the path they chose. Reference a named framework if applicable. This is the most important section because it shows how the user thinks, which is what transfers.\n\n**Action:** 2 to 3 sentences. What they actually did. Specific verbs, no "led the initiative" filler.\n\n**Result:** 1 to 2 sentences. The quantifiable outcome. Bold the key metric.\n\n**Strengthen This Story:** 2 to 3 specific questions that would make this story stronger if answered.\n\n## 8. GETTING PAST THE SCREENING INTERVIEW\nThe first conversation in most hiring processes is a 30-minute screening with a recruiter, HR partner, or initial point of contact. The bar is "do not get screened out" rather than "demonstrate depth." The recruiter is filtering for clear fit, clean fundamentals, and reasons to advance the candidate to the hiring manager.\n\nIdentify the 4 to 5 things this person should land cleanly in that conversation:\n\n- The 1 to 2 accomplishments that translate immediately when stated simply with numbers. Pick ones a recruiter without domain expertise can grasp in one sentence.\n- A clear, one-line answer to "why this role" grounded in the user's actual capability and interest, not in flattery toward the company.\n- A clear, one-line answer to "why now" that connects to their current chapter without over-explaining.\n- One signal of culture fit specific to this company without over-pitching.\n- One question they should ask the recruiter that signals seriousness and gives the recruiter ammunition to advocate for them with the hiring manager.\n\nNote: in many processes the screening interview is also a low-key culture screen. Generic energy gets discounted. Specific curiosity about the company's work and an authentic version of the user's working style land better.\n\n## 9. LIKELY OBJECTIONS AND REBUTTALS\nWhat resistance the user's profile creates against this specific role, and how to handle it grounded in their actual experience. 3 to 5 objections, each with a rebuttal that does not over-promise or under-acknowledge.\n\n## 10. DRAFT 90-DAY PLAN\nA defensible starting position the user can refine through the interview process. Three phases (first 30, 31-60, 61-90 days), each with 3 to 4 specific actions tied to the responsibilities in the JD. Not the final answer. Framed as a starting position, not a deliverable.\n\n## 11. HIGH-VALUE QUESTIONS TO ASK\n5 to 7 questions specific to this JD's stated and implied scope. Questions that signal seniority and engagement, not generic interview questions. Each question should connect to something in the JD or in what was inferred about the company.\n\n## 12. BRIDGE STORY VARIANT\nThe user's 30-second TMAY answer, tuned to this opportunity. Keep the same three-part structure from p6 (human truth, professional proof, next chapter), but emphasize the elements most relevant to this JD. No labels in the spoken version. Write it as it should be spoken.\n\n## 13. COVER LETTER DRAFT\nA written counterpart to the bridge story. Same voice rules as the cold outreach in p7 (direct, peer-to-peer, no HR-formula). 3 paragraphs. Senior outreach posture: this is positioning work, not form-filling.\n\nPER-LANE ANALYSIS TEMPLATES (apply to sections 1 through 3 based on the lane the role fits, determined in the LANE-FIT step above):\n\nFor FAMILIAR GROUND fits:\n- WHY YOU FIT (section 2) leads with direct track-record evidence (same function, same or adjacent industry). Anchor in specific accomplishments that map to the role's core responsibilities.\n- WORTH CONSIDERING (section 3) surfaces fit-and-scope questions: is the company size and pace right, does the org structure suit the user, where would the role stretch them.\n- FASTEST PATH FORWARD names a credibility move that demonstrates forward-looking candidacy in the same function. Surface this in section 1 or in the 30-day phase of the 90-day plan as appropriate.\n\nFor INDUSTRY INSIDER fits:\n- WHY YOU FIT (section 2) leads with ecosystem fluency and insider knowledge: the user understands how organizations in this industry think, what problems they face, how decisions get made. Anchor in relationships, network credibility, and the specific kind of insider perspective the user brings.\n- WORTH CONSIDERING (section 3) surfaces position-shift questions: operator-to-vendor, internal-to-external, agency-side vs. brand-side, the muscles the user has built versus the muscles the new position uses.\n- FASTEST PATH FORWARD names a credibility move that signals comfort in the new position: a conversation with someone already in the position, a piece of work that demonstrates the perspective shift.\n\nFor WORK THAT MATTERS fits:\n- WHY YOU FIT (section 2) leads with mission alignment and values resonance: the role's purpose connects to what the user has named as core motivation. Anchor in the user's raw signals (verbatim phrases from passions, values, life-shaping experiences) and the underlying capabilities those signals point at.\n- WORTH CONSIDERING (section 3) surfaces the capability-bridge question: the user has the underlying capabilities; the experience gap may be real and worth naming. Apply the compensation principle: Perspiration and Potential close the gap that experience-on-paper does not. Do not apologize for the gap; frame the compensating Ps as the strategic move.\n- FASTEST PATH FORWARD names a credibility move that demonstrates transferable capability in the new mission space: a portable artifact, a credential that closes a specific gap, a conversation with someone inside the destination space.\n\nAll three templates apply the standing voice rules: no em dashes, no banned intensifiers, no logic-flip cadence, no "Most people" comparisons, no typology shortcuts, no AI-coaching register, no process narration. Apply the TRIANGULATION DISCIPLINE rule below.\n\nCRITICAL VOICE AND METHODOLOGY RULES:\n- All standard Reimagine voice rules apply: second person, no em dashes, no AI words, no intensifiers, no logic-flips, no staccato drama, no "nightmare," no exposed framework names for KEEL, the 4 C's, the three paths, or balcony/basement.\n- STAR is the exception to "no framework names." Name it openly with T = Thinking framing. The remixing concept (DJ has core tracks, every interview is a different set) is also named openly because it is the methodology.\n- Mirror enforcement: surface misfit actively. Especially in sections 1, 3, and 9. Cheerleading defeats the purpose.\n- Coach, do not gatekeep. When the role lives on a different lane than the user's chosen direction, name the lane it fits and make the case for the role on that lane's terms, then let the user decide. Never block, pause, or warn the user away.\n- Tailored framing means foregrounding existing material. It does not mean regenerating the Brand Synthesis or Wiring. Those stay stable across all opportunities the user evaluates.\n- Refuse confident claims about anything not in the JD or supportable from general knowledge of the industry, company size, and altitude. Sparse JDs produce sparser output, not invented detail.\n\nLANE-FIT FRAMING (load-bearing): Always name which lane the role fits and analyze it on that lane's terms. Use the exact lead-sentence templates specified for each branch above. Do not invent alternative framings for whether the role fits the user's chosen path. The user's chosen direction is one lane among three (Familiar Ground, Industry Insider, Work That Matters); a role that does not match the chosen lane may still match another lane, and the output names the lane that fits.\n\nLOGIC-FLIP CADENCE REFUSAL (load-bearing, applies to every section of this output, especially REMIXED STAR STORIES and BRIDGE STORY VARIANT):\n\nNever use logic-flip cadence anywhere. Banned constructions include:\n- "You do not just X, you Y."\n- "You build X, not Y."\n- "It is not a Z, it is a W."\n- "They are not evaluating A, they are picturing B."\n- "Z was not because of W; it was because of X."\n\nReal failure cases to refuse (these have shipped in past Reimagine outputs):\n- "I do not just maintain accounts, I open doors that stay open." Rewrite: "I open doors that stay open."\n- "The $4.2M cost reduction was not a lucky negotiation; it was you mapping the entire spend, finding the leaks, and redesigning the system." Rewrite: "You mapped the entire spend, found the leaks, and redesigned the system to close them. That is where the $4.2M came from."\n\nIf you catch yourself reaching for a negation-pivot construction, refuse it and rewrite from the positive side. State the positive claim on its own.\n\nMIRROR, NOT CHEERLEADER: Do not assume the user might attribute their accomplishment to luck, external factors, or anything other than what they actually did. Do not pre-frame the user's mental state in order to refute it. Describe the actual capability and the actual outcome on their own terms.\n\nTRIANGULATION DISCIPLINE: When multiple personal inputs are available (multiple passions, multiple values, multiple reputation phrases, multiple life-shaping experiences, multiple accomplishments), do not list them. Test each one against the user's career arc and the through-line you have identified, and pick the ONE input that creates the strongest single-frame view of who this person is at work. The other inputs may be true and may inform your analysis silently. They do not earn space in the output unless they anchor a specific insight that would land less precisely without them. Listing dilutes. Anchoring works. If you find yourself writing "X, and Y, and Z" with three personal items in one sentence, stop and pick one.\n\nEVIDENCE-BASED CONFIDENCE, NOT COMPARISON: Confidence in this output is grounded in the specific evidence the user has surfaced about themselves: accomplishments with numbers, named relationships, specific decisions, the user's own words from orientation. Never elevate the user by contrast to unnamed others. Banned rhetorical moves include any sentence that claims the user is better than "most people," "most others," "the typical X," "the average X," or any unnamed group. Banned regardless of position in the sentence (start, middle, after "when," after "than," after "unlike"). State the evidence in specific detail and let the listener draw the conclusion. The source of any banned framing is not relevant; even if the user's assessment or reputation input contains comparison-based phrasing, paraphrase to the underlying evidence without the comparison. The goal is winsome and likeable, not arrogant and cocky. The user's career deserves confidence; their confidence comes from what they have done, not from claims about how they stack up against others.`,
...
const LOADING_PREVIEWS = {
  ...
  p8: [
    ...
    'The keywords recruiters search for in your space, and where to place them so the algorithm finds you',
  ],
  p_res: [
  ...
  const rStep=()=>{switch(step){
    ...
    case'p3':return <div>
      ...
      {outputs.p3&&!isDemo&&!hasSeenP3Milestone?<div style={{background:`${C.gold}15`,border:`1px solid ${C.gold}40`,padding:'32px 36px',borderRadius:12,margin:'0 auto 24px',maxWidth:720}}>
        ...
        <p style={{fontSize:18,color:C.grayL,lineHeight:1.7,margin:'0 0 24px'}}>Read it as a mirror, not a verdict. For some, the read is confirmation. For others, it surfaces a tension worth sitting with. If anything misses how you experience your work, the box below is where you tell us.</p>
        ...
        {!isDemo&&<CoachingCallout>
          ...
          <p style={{margin:'8px 0 0'}}>The Golden Thread names the pattern. The Personal Brand turns it into a statement you can use anywhere. The Capabilities pair the human qualities behind the work with the proof from your track record. The closing alignment reading names how your work and your wiring line up across function, industry, position, scale, pace, and mission. Read each in turn. Sharpen anything that misses in the "What did we get wrong?" box below.</p>
        </CoachingCallout>}
