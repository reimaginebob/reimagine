...
- Write in a natural, human voice. Avoid words that read as AI-generated: "exactly," "straightforward," "unlock," "leverage," "utilize," "robust," "seamless," "game-changer."
...
function Sidebar({step,done,onNav}){return <div style={{width:200,background:C.panel,borderRight:`1px solid ${C.border}`,padding:'16px 0',overflowY:'auto',flexShrink:0}}>{PHASES.map(ph=><div key={ph.id} style={{marginBottom:3}}><div style={{fontSize:9,fontWeight:800,letterSpacing:'1.5px',textTransform:'uppercase',color:ph.color,padding:'5px 14px 3px',display:'flex',alignItems:'center',gap:5}}><div style={{width:5,height:5,borderRadius:'50%',background:ph.color}}/>{ph.label}</div>{ph.steps.map(sid=>{const active=step===sid,isDone=done.includes(sid),can=isDone||active;return <div key={sid} onClick={()=>can&&onNav(sid)} style={{padding:'6px 14px 6px 28px',display:'flex',alignItems:'center',gap:7,cursor:can?'pointer':'default',background:active?`${ph.color}15`:'transparent',borderLeft:`2px solid ${active?ph.color:'transparent'}`,fontSize:12,color:active?C.cream:isDone?C.grayL:C.gray,transition:'all 0.15s'}}><div style={{width:14,height:14,borderRadius:'50%',border:`1.5px solid ${active?ph.color:isDone?C.ok:C.border}`,background:isDone?C.ok:'transparent',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>{isDone&&<Check size={8} color={C.bg} strokeWidth={3}/>}</div>{META[sid]}</div>})}</div>)}</div>}
