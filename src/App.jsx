          ...
          {lanes.length>0&&<>
            ...
            {activeLane&&<div style={{background:'#FFFFFF',border:`1px solid ${C.border}`,borderRadius:'0 0 12px 12px',borderTop:'none',padding:'28px 30px',marginBottom:16}}>
              ...
              {(()=>{
                ...
                const optBlocks=content.split(/(?=^#{1,3}\s*OPTION:\s*)/m)
                const preamble=optBlocks.length>0&&!/^#{1,3}\s*OPTION:/m.test(optBlocks[0])?optBlocks.shift():null
                if(optBlocks.length===0)return <div style={{fontSize:15,color:'#374258',lineHeight:1.7}}><MD text={content}/></div>
                return <>
                  {preamble&&<div style={{fontSize:15,color:'#374258',lineHeight:1.7,marginBottom:16}}><MD text={preamble}/></div>}
                  {optBlocks.map((block,bi)=>{
                    const titleMatch=block.match(/^#{1,3}\s*OPTION:\s*(.+)/m)
                    const title=titleMatch?titleMatch[1].replace(/\*\*/g,'').trim():null
                    const body=titleMatch?block.slice(block.indexOf('\n',block.indexOf(titleMatch[0]))+1).trim():block
                    const isSelected=title&&deepOpts.includes(title)
                    ...
                    return <div key={bi} style={{marginBottom:16,border:`2px solid ${isSelected?C.gold:C.border}`,borderRadius:12,overflow:'hidden',transition:'all 0.2s'}}>
                      {title&&<button onClick={()=>canSelect&&toggleOpt(title)} style={{display:'flex',alignItems:'center',gap:12,width:'100%',textAlign:'left',padding:'14px 20px',background:isSelected?`${C.gold}12`:'#FAFBFC',border:'none',borderBottom:`1px solid ${isSelected?`${C.gold}30`:C.border}`,cursor:canSelect?'pointer':'not-allowed',opacity:canSelect?1:0.5,fontFamily:'inherit',transition:'all 0.15s'}}>
