...
const LOADING_PREVIEWS = {
  ...
  p5: [
    ...
    'Why each one fits your background, what to think through before committing, and the fastest path in',
    'The honest tradeoffs, not only the upside',
  ],
  p6: [
...
function RefineBox({value,onChange,onRegenerate,hint,placeholder,updateLabel,freshLabel}){
  ...
  return <div style={{marginTop:28,marginBottom:28,border:`2px solid ${C.border}`,borderRadius:12,overflow:'hidden',background:'#F7F8FA'}}>
    ...
    {open&&<div style={{background:'#FFFFFF',padding:'16px 20px',borderTop:`1px solid ${C.border}`}}>
      ...
      <div style={{fontSize:13,color:'#8A9BB8',marginTop:8,fontStyle:'italic'}}>{hasSpeech?'Tip: Tap the microphone to speak, or type. ':''}This is for factual corrections too. If we got something wrong about your experience, your role, or how we read it, tell us. Corrections will apply to future regenerations of other sections as well.</div>
...
<div class="footer"><span>Reimagine by Career Club · career.club</span><span>${esc(date)}</span></div>
  ...
  const rStep=()=>{switch(step){
    ...
    case'linkedin':return <div>
      ...
      <div style={S.card}>
        <p style={{fontSize:13,color:C.gray,fontStyle:'italic',margin:'0 0 10px'}}>PDF, Word, or plain text. The PDF export from LinkedIn works directly.</p>
    ...
    case'p4':return <div>
      ...
      {outputs.p4&&p4Intro&&(()=>{
        ...
        return <div style={{maxWidth:820,margin:'0 auto'}}>
          ...
          <div style={{background:'#FFFFFF',border:`1px solid ${C.border}`,borderLeft:`3px solid ${C.gold}`,padding:'18px 22px',borderRadius:8,margin:'0 0 24px',fontSize:15,color:'#1A2540',lineHeight:1.65}}>
            <strong style={{fontSize:16}}>How this works</strong>
            ...
            <p style={{margin:'0 0 0',fontSize:14,color:C.gray}}>You can change your mind later; roles you do not pick are not lost. If anything here is unclear, ask in the chat in the corner.</p>
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:20,marginBottom:36}}>
