  ...
  const rStep=()=>{switch(step){
    ...
    case'p7':return <div>
      ...
      {outputs.p7&&(()=>{
        ...
        return <>
          <div style={S.out}><div style={{display:'flex',justifyContent:'flex-end',marginBottom:12}}><Btn small onClick={()=>copy(cleanText)}>{copied?<><CheckCheck size={11}/>Copied</>:<><Copy size={11}/>Copy All</>}</Btn></div><MD text={part12}/></div>
          <div style={{margin:'16px 0',padding:'16px 20px',background:`${C.gold}14`,border:`2px solid ${C.gold}60`,borderRadius:10,display:'flex',alignItems:'center',justifyContent:'space-between',gap:16}}>
            ...
            <Btn onClick={()=>{
              const parseCompanies=(text)=>{
                ...
                const companies=[]
                let current=null
                const finalize=()=>{if(current&&(current.fit||current.growth||current.contact||current.emailConvention||current.what||current.industry||current.size||current.hq))companies.push(current);current=null}
                for(const line of text.split('\n')){
                  const trimmed=line.trim()
                  const nameMatch=trimmed.match(/^\*\*([^*]+?)\*\*\.?$/)
                  if(nameMatch){
                    const name=nameMatch[1].trim().replace(/\.$/,'')
                    if(/^PART\s|^Company Name$|^Why it fits|^Growth signal|^Contact|^Email|^The Hook|^The Story|^The Close|^Paragraph\s|^What|^Industry|^Size|^HQ/i.test(name)){finalize();continue}
                    finalize()
                    current={name,what:'',industry:'',size:'',hq:'',fit:'',growth:'',contact:'',contactLinkedIn:'',emailConvention:'',website:''}
                    continue
                  }
                  if(!current)continue
              ...
              const header='Company,What they do,Industry,Size,HQ,Why it fits,Growth signal,Contact name & LinkedIn,Email convention,Website'
              const rows=companies.map(c=>{
                const contactCell=[c.contact,c.contactLinkedIn].filter(Boolean).join(' | ')
                return [c.name,c.what,c.industry,c.size,c.hq,c.fit,c.growth,contactCell,c.emailConvention,c.website].map(esc).join(',')
              })
              const csv=header+'\n'+rows.join('\n')
