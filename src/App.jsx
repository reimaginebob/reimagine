...
function Loading({ msg = 'Generating your analysis…', step = '' }) {
  ...
  return <div style={{textAlign:'center',padding:'48px 24px',maxWidth:640,margin:'0 auto'}}>
    ...
    <div style={{fontSize:15,color:C.gray,marginTop:20}}>{(step==='p6'||step==='p7')?'This may take 4–5 minutes':'This may take 3–4 minutes'}</div>
  </div>
}
...
export default function PivotEngine(){
  ...
  const downloadOnePager=()=>{
    ...
    const p6obj=(outputs.p6&&typeof outputs.p6==='object')?outputs.p6:null
    const whyRemember=(()=>{if(!p6obj||!p6obj.bridge_story)return getSection(bridgeProse,['WHY THEY REMEMBER YOU','WHAT MAKES YOU STICK','THE THREE THINGS']);const slot=p6obj.bridge_story.slot1_human_anchor;if(!slot)return '';const u=(p6obj.user_picks||{}).slot1;if(u&&typeof u.edited_text==='string'&&u.edited_text.trim())return u.edited_text.trim();if(u&&u.picked_id){const o=(slot.options||[]).find(x=>x&&x.id===u.picked_id);if(o&&o.text)return o.text}const f=(slot.options||[])[0];return f&&f.text?f.text:''})()
