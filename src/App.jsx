...
const P={
  ...
  p_res:(pr,outs,sel)=>`Rewrite this resume using the HYBRID FORMAT to target: **${sel}**\n\nPER-PATH RESUME ADAPTATION:\n\nFor FAMILIAR GROUND-bound candidates:\nThe Repositioned Summary leads with the candidate's most-relevant domain experience and the specific function they are pursuing. Key Accomplishments should be selected for direct relevance: domain experience plus function depth plus recent results. The candidate's strongest credibility lives in the resume's most obvious reading, so do not over-engineer the structure. Make the relevance crystal clear in the first three lines.\n\nFor INDUSTRY INSIDER-bound candidates:\nThe Repositioned Summary leads with industry-specific credibility and ecosystem fluency. Key Accomplishments should include at least one story that demonstrates relationships and insider knowledge alongside business outcomes. Industry-specific keywords for ATS and human readers.\n\nFor WORK THAT MATTERS-bound candidates:\nThe Repositioned Summary foregrounds transferable strengths that map to the new domain. Lead with capability framing ("commercial leader who builds revenue systems from scratch") rather than domain framing ("CPG executive"). De-emphasize non-transferable domain detail in the first three lines. Key Accomplishments should be selected for what they reveal about the candidate's portable pattern; direct relevance to the most recent work is secondary. Include at least one accomplishment that demonstrates the transferable strength in a form the destination domain will recognize. Apply the Five Ps compensation principle from the framework memory: Proficiency is light for the new domain so the other four Ps must come through in the resume's voice.\n\nPROFILE: ${outs.p1}\n${outs.p3}\nORIGINAL RESUME:\n${pr.resume}\n\nRAW SIGNALS (this person's own words from orientation, do not paraphrase back to them):\nVALUES: ${pr.values||'not provided'}\nPASSIONS AND CAUSES: ${pr.passions||'not provided'}\nPRAISE THEY RECEIVE: ${pr.rep.memory||'not provided'}\nWHO CALLS THEM IN EMERGENCY: ${pr.rep.emergency||'not provided'}\nHOW PEOPLE DESCRIBE THEIR SUPERPOWER: ${pr.rep.twoWords||'not provided'}\nOTHER REPUTATION DATA: ${pr.rep.other||'not provided'}\nLIFE-SHAPING EXPERIENCES: ${pr.lifeEvents||'not provided'}\nASSESSMENT TYPE: ${pr.assessType||'not provided'}\nASSESSMENT NOTES: ${pr.assess||'not provided'}\n\nThese raw signals are the strongest input for Personality, Passion, and identity grounding. When you need to ground a claim about who this person is or what they care about, reach for verbatim phrases from these signals rather than paraphrasing through the synthesized profile.\n\nUSE THE STRONGEST GROUNDING SOURCE AVAILABLE. When raw signals point to a specific assessment finding, a verbatim reputation phrase, a named passion, or a specific formative life pattern, lead with that. Defaulting to the safer professional-only proof is a failure mode.\n\nBEFORE WRITING THE STRUCTURED OUTPUT, FIND THE FORCE.\n\nPattern recognition is your default mode and a necessary first move. The synthesis the user needs depends on going further: finding the FORCE that integrates their choices, beyond the surface pattern that runs through them.\n\nFour levels of analytical depth. Three to refuse. One to target.\n\nLEVEL 1 (refuse): apply the structural template directly to the inputs. Each input field becomes a slot. The result is generic.\n\nLEVEL 2 (refuse): find the pattern that runs across the inputs. Name it. Treat naming the pattern as the synthesis. This is your default mode. It is a label only.\n\nLEVEL 3 (refuse): pattern with personal source. Find the pattern AND the formative experience or stated value that "explains" the pattern. Write it up as "X is true about this person because Y experience taught them Z." This looks like synthesis. It is pattern recognition with backstory attached. The integration is still missing.\n\nLEVEL 4 (target): force-level synthesis. Find the INTEGRATING FACTOR that explains why the source produced this specific pattern, and predicts what the person will do in situations not yet shown. The force is what makes the user a coherent person, beyond a coherent profile.\n\nTo find the force, ask these questions internally before writing. The first four surface the pattern. The last three surface the force.\n\nPattern (first layer):\n- Why is this person in this field, specifically?\n- Why this specific function within the field?\n- Why have they stayed or moved when they did?\n- What is the conviction, named or implicit, that explains the choices?\n\nForce (the integrating layer):\n- What is this person trying to prove, resolve, repair, or protect through their work? The force usually takes one of these four shapes.\n- What does this person worry about that no one would guess from their resume? The implicit threat the work is structured to prevent.\n- What does this person love about the work that the work itself does not visibly demand? The signal of what the work means to them beyond the job description.\n\nThe signal that the force has been found: recognition. The synthesis surfaces something that was true but unstated, and the user recognizes it as accurate to how they experience their work. If the synthesis only describes what is already visible, you are at Level 2 or 3. Keep working.\n\nDo NOT show your force-finding work in the output. The pre-step is internal analytical discipline. The user reads only the structured output.\n\nFor this output specifically: the force shapes the Repositioned Summary. The summary leads with the through-line in language the user could carry into a conversation.\n\nThe hybrid format puts a Key Accomplishments section above the fold, before the chronological work history. This is the single most important structural choice: hiring managers scan for 7-10 seconds, and your strongest evidence needs to be the first thing they see.\n\nSTART your response with:\n## QUICK TAKEAWAY\n3-4 sentences: the biggest structural change from the original resume, which accomplishments made the Key Accomplishments section and why, and the one thing that will hit a hiring manager in the first 7 seconds. Plain language, no headers inside this section.\n\nThen continue with the full rewrite:\n\n1. REPOSITIONED SUMMARY: 3-4 sentences at the top. First person, natural voice. Positions this career arc as a logical path toward ${sel}. No titles or company names. Frame the pivot as an asset.\n2. KEY ACCOMPLISHMENTS: 3-5 of the most significant accomplishments from anywhere in their career, selected and framed specifically for ${sel}. Each accomplishment uses the XYZ formula: Accomplished X, as measured by Y, by doing Z. Each must connect to one of the three value levers (made money, saved money, mitigated risk). Bold the key metrics. This section sits between the Summary and Work History, above the fold, and serves as the discussion guide for the interview.\n\nPERSPIRATION SIGNALING (apply to candidates with 15+ years of experience):\n\nThe Five Ps framework names Perspiration as one dimension hiring managers evaluate: will this person grind? Senior candidates face an unstated filter that assumes lower energy or hands-off operational presence. The resume needs to neutralize that filter through evidence, not assertion. Never name Perspiration. Never write "energetic" or "high-output" or "hands-on operator" in the resume itself. Surface the signal through the accomplishments you select and how you frame them.\n\nVOLUME SIGNAL: Select accomplishments that show the number of things delivered, not only the size of the largest thing. A senior candidate who has launched 20 brands, opened 5,000 stores, managed 200 accounts, or shipped 40 product lines has a volume signal a single-large-result accomplishment misses. Where the resume supports a count, lead with the count.\n\nDENSITY SIGNAL: Select accomplishments that show concurrent scope. A candidate who managed a $300M P&L AND launched a new product line AND restructured the broker network AND opened a new channel inside the same role period demonstrates density that a candidate who did one of those things does not. Where the resume supports compression of concurrent priorities, use it.\n\nRECENCY SIGNAL: For the senior candidate cohort, lead the accomplishments list with the most recent intense engagement. If the candidate has stepped into a consulting practice or a slower role recently, surface the operational intensity from the prior role first and frame the consulting work as continued depth rather than a wind-down.\n\nOPERATIONAL PRESENCE SIGNAL: Where the resume supports it, surface accomplishments that demonstrate the candidate IN the work, going beyond the strategy layer. Specific deals closed, specific retailers opened, specific products launched. Senior candidates filtered out as "too strategic to do" are often the ones whose resumes describe outcomes without surfacing the candidate's hand in delivering them.\n\nIf the candidate is under 15 years of experience, this block does not apply. Apply only when the resume signals seniority that would trigger the age-bias filter.\n\n3. EXPERIENCE REWRITE: Chronological work history follows. For each role in the last 10 years, rewrite the top 3-4 bullets. Each bullet must: start with an action verb, end with a business result (made money / saved money / mitigated risk), and connect to skills relevant to ${sel}. Flag any bullet where a specific number is missing and suggest what metric to find.\n\nPER-ROLE PERSPIRATION LAYER (for senior candidates):\n\nFor each role's bullet list, ensure the bullets collectively surface the candidate's pace of work in that role, alongside the strategic outcomes. A role described entirely as outcomes ("Drove $300M in revenue growth") implicitly reads as light-touch executive work. The same role described with mixed-level evidence ("Drove $300M in revenue growth. Restructured broker network across 15 regions. Opened Walmart, Costco, Kroger, Target. Led weekly trade promotion review and rationalized $40M annual trade spend.") signals hands-on operational presence behind the headline result.\n\nMix bullets at three altitudes per role:\n1. The headline result (revenue, savings, scope, outcome with number).\n2. The operational moves that produced the result (broker network, trade spend, account-level details, specific functions managed).\n3. The visible-hand evidence (specific accounts opened, specific products launched, specific people hired, specific systems built).\n\nSenior candidates need all three altitudes visible. A resume that lives only at altitude 1 reads as "this person delegates"; a resume that lives only at altitude 3 reads as "this person is too junior." The mix is the signal.\n\n4. SKILLS AND KEYWORDS: List 8-10 keywords a recruiter or hiring manager for ${sel} would search for. Note which are already in the resume and which to add.\n5. FORMATTING NOTES: Remind the user to maintain two versions: an ATS-friendly version (plain, single-column, standard section titles like Work Experience, Education, Skills) and a designed version for direct human contact. White space is essential. Save as PDF to preserve formatting.\n\nHEDGED LANGUAGE AND CO-AUTHOR INVITATIONS:\n\nEvery interpretive claim in this output uses directional, hypothesis-shaped language. The user reads the claim as a hypothesis you are offering for verification. Vocabulary varies naturally across sentences; avoid making the output read formulaic by repeating any single hedged phrasing.\n\nVerdicts to refuse: "You are X," "This means you Y," "Obviously you Z," "We can see that you W," "The data shows you V."\n\nDirectional language to use: "There is a pattern that seems to indicate," "This may suggest," "Often correlates with," "Tends to signal," "We see a pattern of," "This points toward," and natural variations.\n\nAt major judgment-call moments where you are interpreting fit, alignment, or value, invite the user to react. Roughly 1-2 invitations per major output section, varied phrasing, placed at the end of an interpretive claim after the user has absorbed the read. Place one invitation at the end of the Repositioned Summary, inviting the user to react before adopting it as their summary.\n\nRefused invitation patterns: "Did we get it right?" (too binary), "Are you happy with this?" (asks approval), "Do you agree?" (authority frame), "Confirm this is accurate" (bureaucratic). Use natural co-author phrasings the user can read as honest invitation.\n\nThe user is the authority on their own experience. Surface interpretations and ask for reaction. Never assert what the user feels, prefers, or has been thinking before evidence supports the claim.`,
