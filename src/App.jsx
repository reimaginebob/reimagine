...
const LOADING_PREVIEWS = {
  ...
  p6: [
    'A tell-me-about-yourself answer in your voice, calibrated to the direction you picked: a personal opening, your career in action, and why this lane is the natural next chapter',
    'A coaching note when a second strong opening exists in your profile, so you have an alternative for the conversations that call for it',
  ],
  p7: [
...
function Loading({ msg = 'Generating your analysis…', step = '' }) {
  ...
  return <div style={{textAlign:'center',padding:'48px 24px',maxWidth:640,margin:'0 auto'}}>
    ...
    <div style={{fontSize:15,color:C.gray,marginTop:20}}>{step==='p7'?'This may take 4–5 minutes':'This may take 1–2 minutes'}</div>
  </div>
}
...
const SECTION_EXPLAINERS = {
  ...
  p6: {
    subhead: 'A tell-me-about-yourself answer in your voice, calibrated to the direction you picked, with an optional second opening when your profile has one.',
  },
  p9: {
...
export default function PivotEngine(){
  ...
  const generateP6=async({refine=''}={})=>{
    ...
    try{
      ...
      const prompt=correctionsBlock(profile.corrections)+P.p6(pc,outputs,chosen,laneLabel)+(refine?`\n\nNEW CORRECTION FROM THIS SECTION: ${refine}`:'')
      const raw=await callClaude(prompt,{maxTokens:2000})
      out('p6',typeof raw==='string'?raw:'');markDone('p6')
  ...
  const downloadOnePager=()=>{
    ...
    // cards. The p6 output is now plain prose too (with an optional
    // ---COACHING NOTE--- delimiter); split off the coaching note for the
    // print view rather than scanning for legacy section headers.
    ...
    const bridgeFull=bridgeStoryToProse(outputs.p6)
    const bridgeTMAY=bridgeFull.includes('---COACHING NOTE---')?bridgeFull.split('---COACHING NOTE---')[0].trim():bridgeFull
  ...
  const rStep=()=>{switch(step){
    ...
    case'focus':{
      ...
      const gp=(id)=>({
        ...
        p6:()=>P.p6(pc,outputs,chosen,laneLbl),
