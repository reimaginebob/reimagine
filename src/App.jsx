...
THE FIVE PARTS (no headers between them; they flow as one read):
...
What runs through your work is a particular kind of attention: the will to stay inside something while it is breaking, until it stops breaking.
...
The fit across dimensions is mostly confirming. **Function** is operational leadership, named clearly by the work and confirmed by the wiring. **Industry** is open: your pattern has held across three sectors, so the next move is less about which sector and more about which mission. **Position in the value chain** points firmly to operator roles inside the company. Advisory or consulting moves would be a tension, because your reputation answers and your story converge on being inside the building when things break, not advising from outside it. **Pace** is a match. Operationally complex and time-pressured work is what energizes you, and nothing in your inputs suggests an attraction to slower cycles.
...
<div class="footer"><span>Reimagine by Career Club · career.club</span><span>${esc(date)}</span></div>
  ...
  const rStep=()=>{switch(step){
    ...
    case'orientation-done':return <div>
      ...
      <div style={S.row}><Btn secondary onClick={()=>nav('life-events')}><ArrowLeft size={13}/>Back</Btn><Btn onClick={()=>{advance('orientation-done','p3');generateChain()}}>Begin Know Your Value <ChevronRight size={14}/></Btn></div>
    </div>
