          ...
          <div style={{margin:'16px 0',padding:'16px 20px',background:`${C.gold}14`,border:`2px solid ${C.gold}60`,borderRadius:10,display:'flex',alignItems:'center',justifyContent:'space-between',gap:16}}>
            ...
            <Btn onClick={()=>{
              const parseCompanies=(text)=>{
                ...
                const companies=[]
                let current=null
                const finalize=()=>{if(current&&(current.fit||current.growth||current.contact||current.emailConvention))companies.push(current);current=null}
                for(const line of text.split('\n')){
                  ...
                  if(nameMatch){
                    ...
                    finalize()
                    current={name,fit:'',growth:'',contact:'',contactLinkedIn:'',emailConvention:'',website:''}
                    continue
                  }
                  if(!current)continue
              ...
              const header='Company,Why it fits,Growth signal,Contact name & LinkedIn,Email convention,Website'
              const rows=companies.map(c=>{
                const contactCell=[c.contact,c.contactLinkedIn].filter(Boolean).join(' | ')
                return [c.name,c.fit,c.growth,contactCell,c.emailConvention,c.website].map(esc).join(',')
              })
              const csv=header+'\n'+rows.join('\n')
