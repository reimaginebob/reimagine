...
<div class="footer"><span>Reimagine by Career Club · career.club</span><span>${esc(date)}</span></div>
  ...
  const generateGtmCompanyRead=async(companyName,companyData)=>{
    ...
    if(!rec0){setGtmCompanyReadErrors(e=>({...e,[companyName]:'Open this Focus Playbook from My Playbooks first so the Company Read can save against it.'}));return}
      ...
      const renderBody=(id)=>{
        ...
        if(id==='p7'){
          ...
          const _renderGtmCompanyCard=(company,idx)=>{
            ...
            return <div key={company.name+'-'+idx} style={{background:'#FFFFFF',border:`1px solid ${C.border}`,borderRadius:10,padding:'18px 22px',marginBottom:14}}>
              ...
              <div style={{marginTop:14,display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
                <Btn small secondary={crBuilt} disabled={_p7Busy||crBusy} onClick={()=>generateGtmCompanyRead(company.name,company)}>
                  {crBusy?'Building…':crBuilt?<><RotateCcw size={11}/>Rebuild Company Read</>:<><Sparkles size={12}/>Company Read</>}
                </Btn>
                {_p7Busy&&!crBusy&&<span style={{fontSize:13,color:C.gray}}>Wait for the Go-to-Market build to finish before running Company Read on a company.</span>}
              </div>
              {crError&&<div style={{marginTop:10}}><ErrBox msg={crError}/></div>}
              {crBuilt&&<div style={{marginTop:14,paddingTop:14,borderTop:`1px solid ${C.border}`}}><MD text={cached.content}/></div>}
            </div>
          }
          return <>
