...
async function callClaude(prompt, opts={}) {
  ...
  return stripCoachSpeak(stripMetaNarration(stripEmDashes(raw)))
}
...
- Do not change your prose to accommodate the JSON requirement. The prose synthesis stands on its own.`
  ...
  p4:(pr,o1,o2,o3,o3Structured,selectedLane,previousOptions,userFeedback)=>{const _struct=formatStructuredFitRead(o3Structured);const _LB=selectedLane==='wtm'?`
## WORK THAT MATTERS
...
// Foundation B (2026-05-27, PR #84). Formats the outputs.p3_structured
// object into a multi-line block the p4 prompt interpolates as literal
// input. Returns empty string when the structured input is missing,
// schema-invalid, or only partially populated (fewer than four dimensions
// readable) so the prompt falls through to prose-mining behavior. Colon
// separators are deliberate; em dashes are scrubbed from inserted prompt
// text per the standing rule.
const formatStructuredFitRead = (s) => {
  if (!s || typeof s !== 'object') return ''
  const tl = String(s.throughLine || '').trim()
  const dim = s.dimensionalFit
  if (!tl || !dim || typeof dim !== 'object') return ''
  ...
  const reads = dims
    .map(d => dim[d] && dim[d].status && dim[d].read
      ...
      : null)
    .filter(Boolean)
  if (reads.length < 4) return ''
  return `\nTHROUGH-LINE (from Personal Brand, literal): ${tl}\n\nDIMENSIONAL FIT (from Personal Brand, literal):\n${reads.join('\n')}\n`
}

// Foundation C-arch Phase D.1 (2026-05-27, PR #90). Generalized
// synthesis-context helper for downstream prompt consumption. Sibling of
// formatStructuredFitRead, which p4 still uses; this helper extends the
// shape with topAnchors (currently missing from the p4 helper) and emits
// a labeled "PERSONAL BRAND SYNTHESIS (structured)" block that signals to
// the model the values are canonical input from the Personal Brand step,
// not a re-derivation target.
//
// D.1 pilots this helper on p5 (Focus deep dive) and p8 (LinkedIn Remix)
// by AUGMENTING the existing outs.p3 prose injection with the structured
// block, not REPLACING it. The prose dump keeps the triangulation
// paragraphs and dimensional-fit prose for modules that benefit from
// reading the analytical language directly; the structured block gives
// the canonical through-line plus per-axis fit readings plus anchor
// evidence as discrete signals the model can reference without
...
//
// D.2 will migrate p4 to this helper, drop the old formatStructuredFitRead
// alias, and roll out across the remaining 7 downstream modules. Voice-rule
// consolidation into SYS is the other half of D.2.
...
<div class="footer"><span>Reimagine by Career Club · career.club</span><span>${esc(date)}</span></div>
  ...
  const generateLane=async(lane,opts={})=>{
    ...
    try{
      ...
      // outputs.p3_structured is passed as the 5th arg (formatStructuredFitRead
