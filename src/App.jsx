...
<div class="footer"><span>Reimagine by Career Club · career.club</span><span>${esc(date)}</span></div>
  ...
  const runOpLaneInference=async(slotId)=>{
    ...
    const artifact=asText(rec.outputs&&rec.outputs.op)||asText(outputs.op)||''
    if(!jd||!artifact.trim())return
    opLaneInferRef.current[slotId]=true
    setOpLaneInferring(true)
    try{
      const m=artifact.match(/##\s*QUICK TAKEAWAY\s*\n([\s\S]*?)(?=\n---|\n##\s)/i)
      const takeaway=m?m[1].trim():artifact
      const res=await inferLaneForOpportunity(jd,buildOpProfileSummary(),takeaway)
      if(!res)return
