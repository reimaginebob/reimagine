...
// p11 (Interview Prep) canonical keys carrying 2+ underscores — the only ones at
// drift risk. relevance_bridge_draft is nested and hard-required for behavioral
// questions; questions_to_ask is optional (team shape) and would silently drop.
...
function parseP11QuestionJSON(raw,expectedType){
  ...
  if(first===-1||last===-1||last<first)return null
  let obj;try{obj=JSON.parse(repairMangledJsonKeys(s.slice(first,last+1),P11_JSON_KEYS))}catch{return null}
  let q=null
...
`2. For "questions": this replaces the question count above. Produce about 5 questions for EACH person on the team, tagged with that person's seat_id, drawn from their role, title, function, the user's notes on them, and the JD. You may also include up to 2 broadly-applicable questions tagged "seat_id": "core". Keep each person's set tight at about five; My Coach can generate more on demand.\n`+
...
// appending "Find 10 More" batches, so near-miss names ("Acme, Inc." vs "Acme")
// don't slip past and collide the name-keyed companyReads cache / React keys.
...
// first-party citations; per-match outreach notes are a separate voice-carrying
// pass. Nothing is asserted without a source — the fallback is "practice page,
// no name," never a guessed name.
const RECRUITER_CONCURRENCY=3
// Durable cache key for a settled target: function + industry + seniority band.
// Geography is deliberately excluded (geo is a widening lever, not part of the
// match identity), so widening geo does not thrash the key.
...
- Geography (a preference, NOT a hard filter — senior retained search is often national/global): ${c.geo||'(none given)'}

Return TWO tiers:
- "boutique": independent / specialist firms where the firm itself IS the specialty. Point at the firm (or a named principal if that is how the firm presents).
...
- The firm's OWN site is the source of truth for a person's CURRENT title. Name a practice leader only when the firm's own consultant/profile page confirms they hold the role now, and return that page as leaderProfileUrl.
...
{"criteria_echo":"<function; industry; seniority>","matches":[
  {"kind":"boutique","firm":"<name>","specialty":"<one plain sentence on what they specialize in and why it fits>","url":"<firm or principal page>","sourceUrl":"<the page that establishes the specialty>","confidence":"high|medium|low","openSearchSignal":{"description":"<what is open>","sourceUrl":"<link>"}},
  ...
  try{
    ...
    const clean=arr.map(m=>{
      ...
      if(kind==='practice'){
        ...
        return{...base,
          practice:(m.practice||'').slice(0,160),
          practiceUrl:typeof m.practiceUrl==='string'?m.practiceUrl:'',
      ...
      return{...base,confidence:m.confidence==='high'||m.confidence==='medium'||m.confidence==='low'?m.confidence:'low'}
    }).filter(m=>m.firm)
...
${baseFull?`BASE BRIDGE STORY (their role-level tell-me-about-yourself). If it contains a "---COACHING NOTE---" section, that note names the alternate anchor the base run already flagged; treat it as the strongest alternate candidate and develop it with concrete material from the profile below:
${baseFull}`:`NO BASE BRIDGE STORY YET. This person has not built a role-level tell-me-about-yourself. Write one from scratch, directly from the profile below, targeted to this opportunity. Use the same anchor-selection judgment a role-level Bridge Story uses: open on the strongest human anchor in their inputs (a formative experience, a sustained passion, a training or craft, or a named assessment signal), never on a role, title, company, or time-anchor. Then the career manifestation, then the forward move into this specific opportunity.`}
...
PRIMARY STORY:
Write a 30-second tell-me-about-yourself answer ${baseFull?'adapted from the Base Bridge Story':'built from the profile above'} for this specific opportunity. Keep the three-component shape: human anchor, then career manifestation, then forward move. ${baseFull?'The human anchor stays the same as the base story unless this company\'s mission clearly calls for a different one.':'Select the human anchor by strength from the profile above.'} The career manifestation foregrounds the accomplishments most relevant to this role's competencies. The forward move names why this specific company and this specific role, not the role category, and references at least one specific element from the JD (a stated priority, a named challenge, a strategic direction). Shorter and sharper than the general Bridge Story; cut anything that does not earn its space against this opportunity. Output the primary as plain prose, with no header above it.
...
OUTPUT: the outreach email and personalization guide as prose only. No JSON, no headers beyond the email itself, no preamble, no code fences.`,
  p7_part2_more:(sel,part1Text,existingNamesText,focusText)=>`Find 10 ADDITIONAL target companies for someone pursuing **${sel}**, to append to an existing Go-to-Market company list. Ground your choices in the candidate's profile provided above. Search the web to research current, real companies and contacts.

HIRING-EXECUTIVE CONTEXT (Part 1 — keep the new companies consistent with this read):
${part1Text||'(none)'}

COMPANIES ALREADY ON THE LIST — do NOT repeat any of these (match on the company, not just the exact spelling):
${existingNamesText||'(none)'}

${focusText&&focusText.trim()?'USER FOCUS FOR THIS BATCH (bias the 10 toward this):\n'+focusText.trim()+'\n\n':''}Prioritize companies showing growth and investment (recent funding, acquisitions, expansion, headcount growth); flag or drop companies showing contraction (layoffs in the past 12 months, hiring freezes, major leadership departures). When a passion, value, or life-experience signal from the candidate's raw signals connects to a company, name that connection in the fit field using their language; never invent a passion connection that is not there.
...
Each company object uses the same field meanings as the main list: name; what (one plain sentence); industry; size (band or "Size not confirmed"); hq (or "HQ not confirmed"); fit (one sentence, translation-anchored); growth (one sentence growth or contraction-warning signal); contact (name and title, or "Contact not identified"); contactLinkedIn (URL or empty); source (where sourced, with URL or page title); emailConvention (inferred format or empty); website (company URL or empty). Use empty strings for missing values, never null. Return exactly 10 companies, all new, none on the exclude list above.`,
  // recruiter_note: a short, personalized direct-outreach note for ONE recruiter
  // match. Peer-to-peer, the sibling of the Making Your Own Weather outreach email
  // and the Cover Letter (shares DIRECT_OUTREACH_VOICE). There is no posting to
  // address — the argument is "why this practice/person, given their specialty and
  // my background." Grounded in the candidate's Personal Brand + Bridge Story.
  recruiter_note:(pc,brand,bridgeStory,match,sel,laneLabel)=>`Write a short direct-outreach note the candidate can send to an executive-search contact who specializes in their target space. This is a peer-to-peer message from the candidate, in their own voice — NOT a cover letter, NOT an essay, NOT a life story. A recruiter reads it in fifteen seconds.

CANDIDATE DIRECTION: ${sel||'(unspecified)'}${laneLabel?` — lane: ${laneLabel}`:''}
THE CONTACT: ${match.kind==='practice'?`${match.leaderName||'the practice leader'}${match.leaderTitle?`, ${match.leaderTitle}`:''} — ${match.practice||''} practice at ${match.firm||''}`:`${match.firm||'the firm'}`}
WHY THEM: ${match.specialty||''}

Ground the note in the candidate's Personal Brand and Bridge Story below. Make the case for why THIS contact specifically: name the shared function/industry/level fit in one line, connect one concrete thing from the candidate's background to what this contact searches for, and ask to be considered for relevant senior searches. Do NOT claim the recruiter is expecting them, do NOT imply any prior relationship, do NOT invent mutual contacts.

Length: 90-130 words. Plain language. First person, the candidate's own voice.
OUTREACH VOICE: ${DIRECT_OUTREACH_VOICE}
...
// contactSourceNote: a short, muted confidence cue for the GTM contact line,
// derived from the model-emitted `source` origin token (P.p7 starts source with
// "Company site" / "LinkedIn" / "Press" / "News"). A company-site contact reads
// as confirmed; a LinkedIn-surfaced one reads as the LIKELY decision-maker.
// Empty when the origin is unknown, so nothing renders.
function contactSourceNote(src){
  const s=(src||'').toLowerCase()
  if(!s)return ''
  if(/company site|official (site|website)|leadership|about page|team page|\/(about|leadership|team)\b/.test(s))return 'confirmed via company site'
...
function RecruiterMatchRow({m,note,onCopy,copied}){
  if(!m)return null
  const isPractice=m.kind==='practice'
  const named=isPractice&&m.leaderName
  const primaryUrl=isPractice?(named?m.leaderProfileUrl:m.practiceUrl):(m.url||m.sourceUrl)
  const title=isPractice
    ?(named?`${m.leaderName} — ${m.leaderTitle||'Practice Leader'}`:`${m.firm}: ${m.practice||'practice'}`)
    :m.firm
  return <div style={{background:'#FFFFFF',border:`1px solid ${C.border}`,borderRadius:10,marginBottom:14,padding:'16px 20px'}}>
    <div style={{display:'flex',alignItems:'baseline',gap:8,flexWrap:'wrap'}}>
      ...
      {isPractice&&<span style={{fontSize:15,fontWeight:700,color:'#1A2540'}}>{m.firm}</span>}
    </div>
    <div style={{fontSize:18,fontWeight:700,color:'#1A2540',margin:'8px 0 4px'}}>
      {primaryUrl?<a href={primaryUrl} target="_blank" rel="noreferrer" style={{color:'#1A2540'}}>{title}</a>:title}
    </div>
    {m.specialty&&<div style={{fontSize:15,color:'#2D3748',lineHeight:1.55}}>{m.specialty}</div>}
    {isPractice&&!named&&<div style={{fontSize:13,color:C.gray,marginTop:6,fontStyle:'italic'}}>We couldn't confirm the current practice leader from a first-party source, so this links the practice page directly.</div>}
    ...
    {note===undefined
      ?<div style={{fontSize:13,color:C.gray,marginTop:10}}>Writing your outreach note…</div>
      :note?<div style={{marginTop:10,background:C.input,border:`1px solid ${C.border}`,borderRadius:8,padding:'12px 14px'}}>
        <div style={{fontSize:12,fontWeight:700,letterSpacing:'0.5px',textTransform:'uppercase',color:C.gray,marginBottom:6,display:'flex',justifyContent:'space-between',alignItems:'center'}}>Outreach note<Btn small onClick={()=>onCopy(note)}>{copied?<><CheckCheck size={10}/>Copied</>:<><Copy size={10}/>Copy</>}</Btn></div>
        <div style={{fontSize:15,color:'#2D3748',lineHeight:1.6,whiteSpace:'pre-wrap'}}>{note}</div>
      </div>:null}
  </div>
}
...
function RecruitersCard({data,busy,chosen,onGenerate,onWiden,onCopy,copied}){
  ...
  return <div>
    ...
    {(built||matches.length>0)&&<>
      ...
      {matches.map((m,i)=><RecruiterMatchRow key={(m.firm||'')+i} m={m} note={notes[i]} onCopy={onCopy} copied={copied}/>)}
      {busy&&<div style={{fontSize:14,color:C.gray,margin:'4px 0 12px'}}>Still checking…</div>}
      {!busy&&matches.length>0&&matches.length<4&&<div style={{...S.note,background:C.input,border:`1px solid ${C.border}`,color:'#2D3748'}}>Specialty search is a small world. These are the contacts we can stand behind for this exact path. Use the levers below to widen the search.</div>}
      {!busy&&matches.length===0&&<div style={{...S.note,background:C.input,border:`1px solid ${C.border}`,color:'#2D3748'}}>We couldn't confirm a specialist for this exact combination from a source we trust. Widen one of the levers below and we'll look again.</div>}
      {!busy&&<div style={{marginTop:14}}>
        <div style={{fontSize:13,fontWeight:700,letterSpacing:'0.5px',textTransform:'uppercase',color:C.gray,marginBottom:8}}>Widen the search</div>
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          <Btn small secondary onClick={()=>onWiden('industry')}>Widen industry</Btn>
          <Btn small secondary onClick={()=>onWiden('seniority')}>Widen seniority</Btn>
          <Btn small secondary onClick={()=>onWiden('geography')}>Widen geography</Btn>
          <Btn small secondary onClick={()=>onWiden('function')}>Include an adjacent function</Btn>
        </div>
      </div>}
    </>}
  </div>
...
export default function PivotEngine(){
  ...
  // One-time Personal Brand check-in. seenPbCheckin rides in the same autosave
  // blob so the check-in fires once per user across devices. pbCheckinOpenReq is
  // bumped to open the floating coach when it fires; pbCheckinFiredRef guards
  // against a double-fire within a session before the flag persists.
...
// 2026-06-04_localstorage-account-scoping.md.
  ...
  const regenerateGtmPart1=async(correctionText)=>{
    ...
    if(!g)return
    const companyListText=(g.part_2_company_list||[]).slice(0,100).map((c,i)=>(i+1)+'. '+((c&&c.name)||'')+((c&&c.what)?' - '+c.what:'')).join('\n')
  ...
  const regenerateGtmPart3=async(correctionText)=>{
    ...
    const g=parseGtmJSON(outputs.p7)
    if(!g)return
    const companyListText=(g.part_2_company_list||[]).slice(0,100).map((c,i)=>(i+1)+'. '+((c&&c.name)||'')+((c&&c.fit)?' - '+c.fit:'')).join('\n')
  ...
  // "Find 10 More": mirror the Part 1 / Part 3 regen loop, but APPEND a batch of
  // 10 de-duplicated new companies onto part_2_company_list instead of replacing
  // a field. De-dupe by normalized name AND homepage domain, against the existing
  // list and within the batch. Reuses regeneratingP7Slot ('part2more') and
  // p7SlotErrors, so it serializes with the other per-slot regens.
  const regenerateGtmPart2More=async(focusText)=>{
    if(regeneratingP7Slot!==null)return
    const g=parseGtmJSON(outputs.p7)
    if(!g)return
    const existing=Array.isArray(g.part_2_company_list)?g.part_2_company_list:[]
    const existingNamesText=existing.map((c,i)=>(i+1)+'. '+((c&&c.name)||'')).join('\n')
    const seen=new Set(existing.map(c=>normalizeCompanyNameKey(c&&c.name)).filter(Boolean))
    const seenDomains=new Set(existing.map(c=>normalizeOpeningsDomain(c&&c.website)).filter(Boolean))
    setRegeneratingP7Slot('part2more');setP7SlotErrors(e=>({...e,part2more:null}))
    try{
      let fresh=null,refusal=''
      for(let attempt=1;attempt<=3;attempt++){
        const prompt=correctionsBlock(profile.corrections)+P.p7_part2_more(chosen,g.part_1_hiring_executive||'',existingNamesText,focusText||'')+(refusal?`\n\n${refusal}`:'')
        const rawr=await callClaude(prompt,{webSearch:true,maxTokens:6000,profileBlock:buildUserProfileBlock(pc,outputs),step:'p7'})
        ...
        if(!cand||cand.length===0){refusal='Return a non-empty "companies" array of new companies in the exact JSON shape.';continue}
        ...
        if(vio.length){refusal='A voice rule was violated ('+vio[0].name+': "'+String(vio[0].match).slice(0,60)+'"). Rewrite the new companies to comply. Same JSON shape.';continue}
        fresh=cand;break
      }
      if(!fresh){setP7SlotErrors(e=>({...e,part2more:'We could not find 10 more companies. Try again, or add a focus.'}));return}
      const additions=[]
      for(const c of fresh){
        if(!c||!c.name)continue
        const nk=normalizeCompanyNameKey(c.name),dk=normalizeOpeningsDomain(c.website)
        if(nk&&seen.has(nk))continue
        if(dk&&seenDomains.has(dk))continue
        if(nk)seen.add(nk);if(dk)seenDomains.add(dk)
        additions.push({name:c.name||'',what:c.what||'',industry:c.industry||'',size:c.size||'',hq:c.hq||'',fit:c.fit||'',growth:c.growth||'',contact:c.contact||'',contactLinkedIn:c.contactLinkedIn||'',source:c.source||'',emailConvention:c.emailConvention||'',website:c.website||''})
      }
      if(additions.length===0){setP7SlotErrors(e=>({...e,part2more:'Those all came back as companies already on your list. Try again with a focus to steer toward new ones.'}));return}
      setOutputs(o=>{const obj=parseGtmJSON(o.p7);if(!obj)return o;const list=Array.isArray(obj.part_2_company_list)?obj.part_2_company_list:[];return {...o,p7:JSON.stringify({...obj,part_2_company_list:list.concat(additions)},null,2)}})
      setCurrentRoleSaved(false)
...
<div class="footer"><span>Reimagine by Career Club · career.club</span><span>${esc(date)}</span></div>
  ...
  const buildRecruiters=async(overrides={})=>{
    const slotId=currentSavedSlotIdRef.current
    ...
    try{
      ...
      const criteria={
        function:overrides.function||chosen,
        industry:overrides.industry||(industryKey==='default'?'':industryKey),
        seniority:overrides.seniority||inferSeniorityBand(chosen),
        geo:overrides.geo||[profile.loc.city,profile.loc.country].filter(Boolean).join(', '),
      }
      const sig=recruitersSignatureFor(criteria)+(overrides._widen?('|w:'+overrides._widen):'')
      const found=await findRecruiterMatches(criteria)
      if(reqId!==recruitersReqRef.current)return
      const base={signature:sig,criteria,matches:found.matches,notes:{},builtAt:new Date().toISOString()}
      ...
      const idxQueue=found.matches.map((_,i)=>i)
      const worker=async()=>{while(idxQueue.length){
        const i=idxQueue.shift()
        let noteText=''
        try{
          const raw=await callClaude(P.recruiter_note(pc,brand,bridge,found.matches[i],chosen,laneLabel),{maxTokens:700,temperature:0.5,step:'recruiters'})
          noteText=(typeof raw==='string'?raw:'').trim()
        }catch(e){noteText=''}
        if(reqId!==recruitersReqRef.current)return
        setSavedPlaybooks(prev=>prev.map(rec=>(rec.id===slotId&&rec.source==='door1')?{...rec,recruiters:{...(rec.recruiters||base),notes:{...((rec.recruiters||base).notes||{}),[i]:noteText}}}:rec))
      }}
      await Promise.all(Array.from({length:Math.min(RECRUITER_CONCURRENCY,found.matches.length||1)},worker))
  ...
  // Widening levers. Each relaxes one criterion and re-runs discovery; geography
  // widens to the profile country only.
  const widenRecruiters=(lever)=>{
    const o={_widen:lever}
    if(lever==='industry')o.industry='(any adjacent industry)'
    if(lever==='seniority')o.seniority=inferSeniorityBand(chosen)+' and one band above/below'
    if(lever==='geography')o.geo=profile.loc.country||'(any location)'
    if(lever==='function')o.function=chosen+' (and adjacent functions)'
    buildRecruiters(o)
  }
  ...
  const generateOpBridgeStory=async({refine=''}={})=>{
    ...
    // has no role-level Bridge Story yet — Door 2-only users who never completed Focus
    // orientation — build one from raw inputs instead of dead-ending. P.p6_op already
    // carries Personal Brand, resume, orientation, and the JD, so it can write from
    // scratch; the base story, when present, is an enhancement to adapt, not a hard
    // dependency. The card surfaces a non-blocking soft note in the no-base case.
    ...
    // legacy object-shape records fall back to the decoded prose. Empty when no base exists.
    const baseFull=baseStripped?(typeof outputs.p6==='string'?outputs.p6:baseStripped):''
