...
import { findPersonalBrandTailBoundary, parsePersonalBrandTail, validatePersonalBrandTailSchema, extractLeadSentence } from "./personal-brand-tail.mjs"
...
export default function PivotEngine(){
  ...
  const generateChain=async()=>{
    ...
    try{
      ...
      const o3=await callClaudeWithVoiceGate(()=>correctionsBlock(profile.corrections)+P.p3(pc,o1,o2),{},{step:'p3',onEvent:logVoiceEvent,onStructured:p=>{structuredP3=p}})
  ...
  const refreshP3=async(extraContext='')=>{
    ...
    try{
      ...
      let structuredP3=null
      const o3=await callClaudeWithVoiceGate(fn,{},{step:'p3',onEvent:logVoiceEvent,onStructured:p=>{structuredP3=p}})
      out('p3',o3,{structured:structuredP3})
    }catch(e){setErr(e.message)}
    finally{setLoading(false);setLoadingStage('');scrollToOutput('p3')}
  }
  ...
  const signOut=async()=>{
    ...
    // Belt-and-suspenders: clear any queued migration modal so the "Save your
    // work across devices" CTA does not appear during the sign-out flow.
    // The pe_has_signed_in_before flag set by /api/me already gates the
    // mount-time useEffect; this guards against a timing edge case where the
    // modal was queued before the flag was read.
