...
1. **Pursuing a specific opportunity?** Points to Add an Opportunity in the sidebar for any role you want a tailored playbook for. (Detailed below.)
...
- **Resume Refresh**. Open your resume document and apply the changes. The summary, the career highlights, the role expansion and compression guidance, and a Skills section built from the skills you validated, so the resume carries the keywords an applicant tracking system scans for. Your strongest wins appear both in the Career Highlights block up top and under the jobs where they happened, written from two different angles so they read as evidence rather than repetition. Download it as a recruiter-facing Human version or an ATS version for online applications (see below).
...
Some applications go to a person first. Many go through applicant tracking software (names you may see: Workday, Greenhouse, iCIMS) that reads your resume into fields and scores it for keywords before any human sees it. The two readers reward different things, so Resume Refresh gives you both from the same content, behind a simple toggle above the resume preview.
...
A few minutes to set up. A few minutes of audio to listen back. The effect tends to be grounding, often surprisingly so. The moment Reimagine's output stops being words on a page and starts feeling like a story you can carry into a conversation.
