...
- **Compensation Read** — a sourced pay range for this role and market, triangulated across public salary sites (Glassdoor, BuiltIn, Salary.com, PayScale, and similar), with every dollar figure a clickable link to its source so you can check it yourself. On an Opportunity it uses what it knows about the role — the company, industry, and seniority level from the posting — to anchor to the right segment, and it sets aside sources that are matching a mislabeled version of the role (for example, manager-level pay showing up under a chief-officer title) so a stray outlier doesn't distort the range. The relevant public sources still disagree among themselves, so the card names the honest spread across them rather than one falsely precise number, and it uses live web search. It is a starting point for your own research, not a verdict — a disclaimer under the card says exactly that. Build it when you want it.
