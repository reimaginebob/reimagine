...
export const FEATURE_MAP = [
  ...
  { slug: 'my-pipeline',          reach: 'standalone',  labelId: 'pipeline',
    does: 'is the daily home for a live search: every saved Opportunity Playbook with where it stands, when they next talk, the next step they are taking, and which cards are built, ordered so whatever needs attention comes first. A next step whose date has passed is flagged Overdue; Mark done clears the step, its date, and the flag together and files what they finished onto that opportunity\'s Notes. THIS IS WHERE EVERY OPPORTUNITY LIVES, on every track, and it is the only screen that lists them: when someone asks where an opportunity, a company, or a job they added has gone, the answer is My Pipeline, never My Playbooks. My Playbooks holds the Focus Playbooks, the roles they explored from Career Paths, and nothing else. Someone who added an opportunity before 2026-08-30 last saw it under My Playbooks and will look there first, so say plainly that opportunities moved rather than only naming the new screen. They keep it current by editing a card or by telling the coach what changed, and accepting the one-tap save' },
  ...
  { slug: 'playbook-markdown',    reach: 'focus-gated', label: 'Markdown download',
    ...
    where: 'on each Focus Playbook card in My Playbooks. It is not offered on an opportunity: for those, the Copy buttons on each section and Save Playbook as PDF are the ways out, and Resume Refresh has its own Download as Word' },
