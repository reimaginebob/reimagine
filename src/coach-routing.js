...
export const FEATURE_MAP = [
  ...
  { slug: 'known-contacts',       reach: 'opportunity-gated', label: 'Who You Know Here',
    does: 'shows which of the person\'s own LinkedIn connections already work at the company on an opportunity, so they can ask for an introduction instead of applying cold, and drafts the note to send. It works only after they load their LinkedIn connections export into Reimagine themselves — a file they request from LinkedIn on a computer, which can take up to two days to arrive. That file is read in their browser and never reaches Reimagine, so you cannot see it, cannot see who they know, and must ask rather than assume. It matches first-degree connections only: LinkedIn exports no data about who their connections are connected to, so second-degree introductions are handed off to a pre-filtered LinkedIn search link on the card rather than computed. That hand-off is stronger than it sounds and worth telling people about: on LinkedIn\'s own results page every person carries a "mutual connections" line, which is the introduction path — so the move is to find someone senior enough to matter there, see who they and the person both know, and ask that shared connection for the introduction. A blank result can mean an out-of-date file as easily as no connection. The draft note comes in two versions from one build — an email and a shorter LinkedIn message — and the ask in both is deliberately "who owns this role", never "will you refer me", because a name is cheap for the recipient to give and a referral request often gets no reply. Email is the version to push: a LinkedIn message only arrives if that person happens to be checking LinkedIn. When LinkedIn did not export the address (most connections withhold it), the card explains the common employer conventions, takes the company\'s mail domain from the person, lays out the likely address forms to try, and points at Hunter.io to confirm one — it never presents a guessed address as known. Drafting a note is the one point at which anything about a connection leaves the device — the person\'s first name, title and connection date, all of it public on their LinkedIn profile — while the connections file itself is still never uploaded and the drafts are kept on the device too',
