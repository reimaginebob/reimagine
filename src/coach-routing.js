...
export const FEATURE_MAP = [
  ...
  { slug: 'opportunity-playbook', reach: 'standalone',  labelId: 'op',         parent: 'twoDoors',
    does: 'turns one specific live opening into a tailored plan of attack. Every one a person builds is listed in My Pipeline, on every track, and My Pipeline is also where they add another; they are NOT under Focus Playbooks, the sidebar entry that was called My Playbooks until 2026-09-01, which holds the roles they explored from Career Paths only. On the Go Independent track these hold prospective clients rather than job openings: a person can open one before they have spoken to anyone or after a first conversation, writing or speaking whatever they know so far. "Where this stands" then gives a fast read -- which of the four selling conversations they are actually in, the one move to make this week, at most three things to find out first ranked by what each unblocks, and anything worth changing in how they are selling it. "The full playbook" is the deeper build for a live one, covering what the client actually needs, whether it is worth pursuing, who is really deciding, what scoping has to settle, three ways to shape the work, what to charge and when to name it, the objections, and a follow-up note. Both run on the same sequence the business-of-consulting chapters teach, so what they read here agrees with what you tell them. Each opportunity also keeps a Notes card, where they can write anything worth remembering about it, keep a reply of yours they chose to save, and collect the steps they have finished. Those notes are private to them: nothing there feeds what Reimagine generates, and you never see them, so ask what they have written rather than assuming' },

  { slug: 'your-stories',        reach: 'standalone',  labelId: 'stories',
    does: 'holds the handful of STAR stories they tell in interviews, which the book puts at roughly twelve covering the range of what gets asked. The first set is built from what Orientation already holds, and the parts come from different places: the resume carries Situation and Result, the reputation answers and career pattern carry the Thought Process. Nothing is invented -- where an input does not support a story they get the question instead of a guess, and every part is editable in place because their words win. Remember the T here is Thought Process, never Task: that is the change this method makes to STAR and the reason its answers land. This is the set Interview Prep remixes from when it prepares them for named people. The screen is organised as thirteen questions. Six want a story, and each gets ONE answer with any other candidates collapsed under it, so there is never a pile of competing versions. The weakness question is one of the six but is NOT a STAR story: it has its own panel built on the book\'s Real / Addressed / Ongoing model, grounded in whatever their assessment actually says, and handed to you to work through rather than written for them. Forcing it into a STAR shape is what made an earlier version invent an event, so never coach it as a story. The other seven questions want something besides a story. Five come to you -- tell me about yourself, what is not on your resume, why they left their last role, what they liked most and least, and whether they are over or underqualified -- and two need an Opportunity Playbook, since company research and per-person questions live there. The screen also teaches five frameworks for giving a Thought Process a shape (What So What Now What; People Process Technology; Right People Doing the Right Things the Right Way; Vision Alignment Execution; and ARC), asks which is theirs, and suggests one on a story where the shape fits -- always as an offer, never as something they did. They can also paste in stories written elsewhere, in any format: those get filed under the question each answers, and a Task sitting where the Thought Process belongs is moved or asked about rather than left. Every story carries a "Does this feel right?" box that reworks that one story from their note; a story Reimagine built has no Remove, because correcting it beats deleting it' },
  ...
  { slug: 'resume-refresh',       reach: 'focus-gated', labelId: 'p_res',
    does: 'repoints the resume at a chosen direction, with three versions behind a toggle: a Human version for recruiters, a Traditional version (a classic, familiar layout with no highlights block, for a reader who wants the resume they already know how to skim), and an ATS version for online applications; the top Career Highlights and the body bullets are written to complement, not repeat, each other. Called "Your One-Sheet" on the Go Independent track, where it reads as a credentials document rather than a job application' },
  // Cover Letter (cover-letter-length-register brief 2026-09-16): a card, not a
  // nav step, so it carries its own label/where like compensation-read below.
  // Had no FEATURE_MAP entry before this brief -- adding the Length/Tone
  // controls is a user-facing change, so this fills that gap rather than
  // leaving Coach unable to describe the card at all.
  { slug: 'cover-letter',         reach: 'opportunity-gated', label: 'Cover Letter',
    does: 'drafts a short cover letter tuned to a specific posting, built from the same Personal Brand, Where You Fit, and Resume Refresh material behind the rest of the packet -- a peer-to-peer note, not a formal essay. Two controls sit on the card: Length (Brief, about 75 words; Standard, about 120 words; Full, about 185 words -- each its own shape rather than a shorter version of the same three paragraphs) and Tone (Conversational or Professional). Both default to Full and Professional, which is what the card always produced before the controls existed, so nobody\'s letter changes shape unless they touch it. The choice is per-account, not per-opportunity: pick a setting once and it carries to every cover letter this person builds afterward, on any opportunity, until they change it again. Changing either and rebuilding regenerates the current letter at the new setting right away',
    where: 'a card on the Opportunity Playbook, after Resume Refresh' },
  { slug: 'interview-prep',       reach: 'focus-gated', labelId: 'p11',
    does: 'works the likely interview questions with worked-through answers. Once the person has named anyone in Interview Team it changes shape entirely — it prepares them for each conversation by name rather than for the role in general, and marks each interviewer with the one SCOPE dimension they are mainly reading for (Strategy, Culture, Oneself, Passion, Expertise) plus a line on why, and then one more thing to consider — aimed deliberately at what they would NOT have thought of, since the first is drawn from what they already wrote and mostly confirms what they already think. That second line may be another dimension, another face of the same one, or what follows a step or two on from their own read, and it is left blank when there is nothing worth adding. All of it is written as a read rather than as fact, because it is inferred from a seat and from what the person wrote down, and nobody has spoken to these interviewers: hold that same line yourself and never tell someone what an interviewer WILL ask. Each story on a card can also be drafted in full with a chosen SCOPE emphasis, so the same story can be heard as a Strategy answer and then as a Culture one, which is the remix made concrete. It also gives them two or three questions to ask each interviewer, and those follow a rule worth coaching directly because it is the part candidates most often waste: a question that could be asked of anyone on the panel at any company is worse than asking nothing, since the questions are read as evidence of how much thought went in. The first test is whether only THIS person could answer it -- the operations partner watched what his sites lost, the peer already ran the same change and knows where it broke, the recruiter knows why the last candidates did not advance. The second is that a question also reveals what the asker cares about, so pointing it at the dimension that person is weighing does double work. Never anything the company research already answers, and never a statement dressed as a question. If someone shows you the questions they plan to ask, that test is the useful thing to say back. So when someone has a panel, do not teach SCOPE from scratch as though it were new: it is already on their screen, named per person, and the useful conversation is which story to lead with for a given lens and what to push to the front of it — but it does NOT refresh itself when a name is added, so someone who has just filled in their panel needs to rebuild it, and that is worth telling them plainly. Each interviewer also carries an optional Research this person on the web button, which runs a live public-web search on that person (by name, title, and company) and returns a short, sourced summary as an unconfirmed draft for the person to confirm or edit — this is real live research, not a read of what they already typed, and it is the answer whenever someone asks whether Reimagine can look up a specific interviewer.; under each question a "Practice this answer" box lets the person speak or type their own answer and send it here for written feedback on it. Called "Discovery Call & Pitch Prep" on the Go Independent track, where the questions are the ones a prospect asks' },
  ...
  { slug: 'offer-comparison',     reach: 'opportunity-gated', label: 'Compare offers',
    ...
    where: 'on My Pipeline, once two or more opportunities have a logged offer' },
  { slug: 'playbook-markdown',    reach: 'focus-gated', label: 'Markdown download',
    ...
    where: 'on each card under Focus Playbooks, the sidebar entry that was called My Playbooks until 2026-09-01. It is not offered on an opportunity: for those, the Copy buttons on each section and Save Playbook as PDF are the ways out, and Resume Refresh has its own Download as Word' },
