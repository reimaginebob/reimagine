...
export const FEATURE_MAP = [
  ...
  { slug: 'groups-for-this-path',  reach: 'focus-gated', labelId: 'groups',
    does: 'finds the professional communities and career networks for the direction someone is moving into — the local chapter of a trade body, a national association, a practitioner-run career network, a members-only Slack, a meetup. It is the opposite number of Recruiters for This Path: a recruiter places people into the role, this is the profession itself, and it is what to reach for when no recruiter has a search open. What it is FOR matters more than the list: most of the value arrives before any opportunity does. Someone who joins a chapter in a field they are moving toward hears how practitioners actually talk about the work, meets the suppliers, consultants and partners around it and so sees the ecosystem rather than one job title, and gets to say out loud what they are considering and hear back on how their background reads and where those people would point it. That is information gathering and network building, and it is most valuable on the Industry Insider and Work That Matters paths, where the person is moving toward a field rather than staying in one. A role may follow; it tends to arrive last. Say that when someone asks whether these are worth the time. It searches twice, once near their city and once with no geography at all, because the best answer is often a national or online community with no local chapter to find. It names organizations and links their own pages instead of promising a meeting date, and every row states what it costs. It appears on a Focus Playbook and on every Opportunity Playbook, and two playbooks in the same function and industry share one list, so the second one opens already built' },
